...
async def sms(ctx, phone, amount:int):
    ...
    if (amount < 151):
      ...
      embes = discord.Embed(title="PABUSX2", description="Bot by : PABUSX2",color=0xff4612)
      ...
      await ctx.channel.send("ห้ามยิงเกิน150คับไอ่บือ")
...
@bot.command()
async def help(ctx):
  emBed = discord.Embed(title="Bot BY: PABUSX2",description="วิธิใช้",color=0xff4612)
